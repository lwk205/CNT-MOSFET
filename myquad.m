function [Q,fcnt] = myquad(funfcn,a,b,tol,trace,maxfcnt,varargin)
...
if nargin < 6 | isempty(maxfcnt), maxfcnt = 10000; end;
...
[Q(1,:,:,:),fcnt,warn(1)] = ...
   quadstep(f,x(1),x(3),squeeze(y1),squeeze(y2),squeeze(y3),tol,trace,maxfcnt,fcnt,hmin,varargin{:});
[Q(2,:,:,:),fcnt,warn(2)] = ...
   quadstep(f,x(3),x(5),squeeze(y3),squeeze(y4),squeeze(y5),tol,trace,maxfcnt,fcnt,hmin,varargin{:});
[Q(3,:,:,:),fcnt,warn(3)] = ...
   quadstep(f,x(5),x(7),squeeze(y5),squeeze(y6),squeeze(y7),tol,trace,maxfcnt,fcnt,hmin,varargin{:});
Q = squeeze(sum(Q,1));
warn = max(warn);
...
function [Q,fcnt,warn] = quadstep (f,a,b,fa,fc,fb,tol,trace,maxfcnt,fcnt,hmin,varargin)
...
% Subdivide into two subintervals.
   ...
   [Qac,fcnt,warnac] = quadstep(f,a,c,fa,fd,fc,tol,trace,maxfcnt,fcnt,hmin,varargin{:});
   [Qcb,fcnt,warncb] = quadstep(f,c,b,fc,fe,fb,tol,trace,maxfcnt,fcnt,hmin,varargin{:});
   Q = Qac + Qcb;
