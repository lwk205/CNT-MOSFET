function [Q,fcnt] = myquad2(funfcn,a,b,tol1,tol2,trace,maxfcnt,varargin)
% myquad2.m
% function [Q,fcnt] = myquad2(funfcn,a,b,tol1,tol2,trace,varargin)
% This is an extension of myquad.m. Here, first two rows of Q is optimized
% with tol1 and rest of the rows with tol2.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
f = fcnchk(funfcn);
if nargin < 4 | isempty(tol1), tol1 = 1.e-6; end;
if nargin < 5 | isempty(tol2), tol2 = 1.e-6; end;
if nargin < 6 | isempty(trace), trace = 0; end;
if nargin < 7 | isempty(maxfcnt), maxfcnt = 10000; end;
...
[Q(1,:,:,:),fcnt,warn(1)] = ...
   quadstep(f,x(1),x(3),squeeze(y1),squeeze(y2),squeeze(y3),tol1,tol2,trace,maxfcnt,fcnt,hmin,varargin{:});
[Q(2,:,:,:),fcnt,warn(2)] = ...
   quadstep(f,x(3),x(5),squeeze(y3),squeeze(y4),squeeze(y5),tol1,tol2,trace,maxfcnt,fcnt,hmin,varargin{:});
[Q(3,:,:,:),fcnt,warn(3)] = ...
   quadstep(f,x(5),x(7),squeeze(y5),squeeze(y6),squeeze(y7),tol1,tol2,trace,maxfcnt,fcnt,hmin,varargin{:});
Q = squeeze(sum(Q,1));
warn = max(warn);
...
function [Q,fcnt,warn] = quadstep (f,a,b,fa,fc,fb,tol1,tol2,trace,maxfcnt,fcnt,hmin,varargin)
...
if ( max(max(abs( Q2(1:2,:) - Q(1:2,:) ))) <= tol1 ) & ( max(max(abs( Q2(3:end,:) - Q(3:end,:) ))) <= tol2 ) 
   warn = 0;
   return

% Subdivide into two subintervals.
else
   [Qac,fcnt,warnac] = quadstep(f,a,c,fa,fd,fc,tol1,tol2,trace,maxfcnt,fcnt,hmin,varargin{:});
   [Qcb,fcnt,warncb] = quadstep(f,c,b,fc,fe,fb,tol1,tol2,trace,maxfcnt,fcnt,hmin,varargin{:});
   Q = Qac + Qcb;
